...
function isMediaElement(el: any) {
  return el && !el.isDeleted && el.type === 'image';
}
  ...
  ({ canvasId, theme, initialData, onDataChange, onImageToInput, onThemeChange }, ref) => {
    ...
    useEffect(() => {
      ...
      const processMenu = (contextMenu: Element) => {
        ...
        try {
          (contextMenu as HTMLElement).classList.add('canvasflow-excalidraw-menu');
        } catch {
          // ignore
        }
        ...
        if (onImageToInput) {
          ...
          menuItem.className = 'context-menu-item canvasflow-context-menu-item';
        ...
        const allSeparators = contextMenu.querySelectorAll(
          'hr, [role="separator"], .canvasflow-context-separator'
        );
        ...
        const menuItems = contextMenu.querySelectorAll(
          '[role="menuitem"], .context-menu-item, button'
        );
      ...
      <div className="excalidraw-host" data-canvas-id={canvasId}>
        <Excalidraw
          ...
          onChange={(elements: readonly any[], appState: any, files: any) => {
            const nextTheme =
              appState?.theme === 'light' ? 'light' : appState?.theme === 'dark' ? 'dark' : null;
